...
//   index: number;
//   isEditable?: boolean;
//   onCreateLesson?: (chapterId: Id<"chapters">) => void;
...
//     e.stopPropagation();
//     onDeleteLesson?.(lesson._id, lesson.title);
//   };
...
//         {lessonIndex + 1}
//       </div>
//       <p className="text-base text-[#195a5a] flex-grow">{lesson.title}</p>
//       {isEditable && (
//         <Button
//           variant="destructive"
//           size="icon"
//           className="bg-red-500/80 hover:bg-red-600 text-white rounded-md w-8 h-8 flex-shrink-0"
...
//             <div className="flex items-center gap-2">
//               <div className="w-8 h-8 rounded-full bg-[#2a7b7b]/80 text-white flex items-center justify-center font-medium text-sm">{lesson.order}</div>
//               <Link href={`/admin/courses/${courseId}/lessons/${lesson._id}`} className="text-lg font-semibold text-[#195a5a] hover:underline">{lesson.title}</Link>
//             </div>
//             <Button
//               variant="destructive"
//               size="icon"
...
// // SortableChapter component for structure view
...
//               <div className="w-10 h-10 rounded-full bg-[#195a5a] text-white flex items-center justify-center font-bold text-lg shadow-md mr-2">
//                 {chapter.order}
//               </div>
...
//                 </div>
//                 <Button
//                   className="w-full mb-4 bg-gradient-to-r from-[#195a5a] to-[#2a7b7b] text-white hover:from-[#2a7b7b] hover:to-[#195a5a] transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1 text-lg font-semibold py-6 rounded-xl"
//                   onClick={() => router.push(`/courses/${id}`)}
//                 >
//                   Go to watch this course
...
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { toast } from "sonner";  // Update to sonner import
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id && over?.id && lessons) {
      ...
      try {
        ...
        toast.success("Lessons Reordered", { description: "Lesson order has been updated." });  // Update to sonner
      } catch {
        toast.error("Error", { description: "Failed to reorder lessons. Please try again." });  // Update to sonner
      }
    }
  };
...
export default function CourseDetailPage() {
  ...
  const handleSaveChapter = async () => {
    if (!id || !chapterTitle.trim()) {
      toast.error("Error", { description: "Please enter a valid chapter name." });  // Update to sonner
      return;
    }
    try {
      await createChapter({
      ...
      toast.success("Chapter Created", { description: `Chapter "${chapterTitle}" has been added.` });  // Update to sonner
      setIsCreatingChapter(false);
      setChapterTitle("");
    } catch {
      toast.error("Error", { description: "Failed to create chapter. Please try again." });  // Update to sonner
    }
  };

  const handleSaveLesson = async () => {
    if (!creatingLessonChapterId || !lessonTitle.trim()) {
      toast.error("Error", { description: "Please enter a valid lesson name." });  // Update to sonner
      return;
    }
    try {
      await createLesson({
      ...
      toast.success("Lesson Created", { description: `Lesson "${lessonTitle}" has been added.` });  // Update to sonner
      setCreatingLessonChapterId(null);
      setLessonTitle("");
    } catch {
      toast.error("Error", { description: "Failed to create lesson. Please try again." });  // Update to sonner
    }
  };
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id && over?.id && chapters) {
      ...
      try {
        ...
        toast.success("Chapters Reordered", { description: "Chapter order has been updated." });  // Update to sonner
      } catch {
        toast.error("Error", { description: "Failed to reorder chapters. Please try again." });  // Update to sonner
      }
    }
  };
  ...
  const handleDeleteChapter = async (chapterId: Id<"chapters">) => {
    try {
      await deleteChapter({ chapterId });
      toast.success("Chapter Deleted", { description: "The chapter has been successfully deleted." });  // Update to sonner
      setConfirmingDeleteChapterId(null);
    } catch {
      toast.error("Error", { description: "Failed to delete chapter. Please try again." });  // Update to sonner
    }
  };
  ...
  const handleDeleteLesson = async (lessonId: Id<"lessons">) => {
    try {
      await deleteLesson({ lessonId });
      toast.success("Lesson Deleted", { description: "The lesson has been successfully deleted." });  // Update to sonner
      setConfirmingDeleteLesson(null);
    } catch {
      toast.error("Error", { description: "Failed to delete lesson. Please try again." });  // Update to sonner
    }
  };
