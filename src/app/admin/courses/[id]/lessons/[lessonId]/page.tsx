...
export default function LessonPage() {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (file && file.type.startsWith("video/")) {
      ...
      toast.error("Please upload a video file.", { description: "Invalid file" });  // No 'title', use message as main, description optional
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Lesson Updated", { description: "Your lesson has been successfully updated." });  // Message as title, description as secondary
      router.push(`/courses/${courseId}?lesson=${lessonId}`);
    } catch (error) {
      console.error("Error updating lesson:", error);
      toast.error("Error", { description: "Failed to update lesson. Please try again." });  // Message as title, description as secondary
    } finally {
      setPending(false);
    }
  };
