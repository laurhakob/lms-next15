...
export default function CourseWatchPage() {
  ...
  const handleToggleComplete = async (lessonId: Id<"lessons">) => {
    try {
      ...
      const updatedCompleted = !progressMap.get(lessonId);
      progressMap.set(lessonId, updatedCompleted);
      toast.success(updatedCompleted ? "Lesson Completed" : "Lesson Uncompleted", { description: updatedCompleted ? "You've marked this lesson as complete." : "You've marked this lesson as incomplete." });  // Update to sonner
      if (lessonId === selectedLessonId) {
        // Update the button for the selected lesson if needed
      }
    } catch {
      toast.error("Error", { description: "Failed to update lesson status. Please try again." });  // Update to sonner
    }
  };
